    ...
    public class KnjiznoZaduzenje
    {
        public static async Task<XmlDocument> Generate(string invoiceNumber)
        {
            ...
            InvoiceCreator.CreateComplexElement(ref document, ref rootElement, new ComplexInvoiceElement
            {
                ...
                {
                    !hasDocumentReference ? (new SimpleInvoiceElement { Name = "StartDate", Value = invoice.InvoicePeriodStartDate.ToString("yyyy-MM-dd") }) : null,
                    !hasDocumentReference ? (new SimpleInvoiceElement { Name = "EndDate", Value = invoice.InvoicePeriodEndDate.ToString("yyyy-MM-dd") }) : null,
            ...
            // Billing reference
            // Da li se odnosi na period ili dokument
            ...
            foreach (var invoiceTax in invoice.InvoiceTaxes)
            {
                InvoiceCreator.CreateComplexElement(ref document, ref taxElement, new ComplexInvoiceElement
                {
                    ...
                    Elements = new List<InvoiceElement>
                    {
                        ...
                        new SimpleInvoiceElement { Name = "TaxAmount", Value = invoiceTax.TaxSubtotalAmount.ToString("0.00", CultureInfo.InvariantCulture), Attributes = new List<InvoiceElementAttribute> { new InvoiceElementAttribute { Name = "currencyID", Value = "RSD" }}},
            ...
            foreach (var item in invoice.InvoiceItems)
            {
                ...
                InvoiceCreator.CreateComplexElement(ref document, ref rootElement, new ComplexInvoiceElement
                {
                    ...
                    Elements = new List<InvoiceElement>
                    {
                        ...
                        // Kolicina
                        new SimpleInvoiceElement { Name = "CreditedQuantity", Value = item.InvoicedQuantity.ToString("0.00", CultureInfo.InvariantCulture), Attributes = new List<InvoiceElementAttribute> { new InvoiceElementAttribute { Name = "unitCode", Value = "H87" }}},
            ...
            if (saveXml)
            {
                document.Save(xmlLocation + $"document_knjizno_zaduzenje_{invoice.InternalInvoiceID}.xml");
            }
